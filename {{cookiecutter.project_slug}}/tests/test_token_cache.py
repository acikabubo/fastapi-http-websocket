...
async def test_token_cache_hit():
    ...
    with patch(
        "{{cookiecutter.module_name}}.utils.token_cache.get_redis_connection",
...
async def test_token_cache_redis_unavailable():
    ...
    with patch(
        "{{cookiecutter.module_name}}.utils.token_cache.get_redis_connection", return_value=None
    ):
        cached_claims = await get_cached_token_claims(token)
...
async def test_token_cache_metrics_hit():
    ...
    with (
        ...
        patch("{{cookiecutter.module_name}}.utils.metrics.token_cache_hits_total") as mock_hits,
    ):
...
async def test_token_cache_metrics_miss():
    ...
    with (
        ...
        patch("{{cookiecutter.module_name}}.utils.metrics.token_cache_misses_total") as mock_misses,
    ):
        await get_cached_token_claims(token)
