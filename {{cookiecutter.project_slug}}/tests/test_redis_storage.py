...
from {{cookiecutter.module_name}}.storage.redis import (
    REventHandler,
    RedisHandler,
    RedisPool,
    RRedis,
...
class TestRedisPool:
    ...
    async def test_get_instance_creates_new(self):
        ...
        with (
            patch("{{cookiecutter.module_name}}.storage.redis.ConnectionPool.from_url") as mock_pool,
            patch(
                "{{cookiecutter.module_name}}.storage.redis.Redis.from_pool",
    ...
    async def test_add_kc_user_session(self, mock_redis, mock_user):
        ...
        """
        with patch("{{cookiecutter.module_name}}.storage.redis.app_settings") as mock_settings:
...
class TestGetRedisConnection:
    ...
    async def test_get_auth_redis_connection(self):
        ...
        with (
            patch("{{cookiecutter.module_name}}.storage.redis.get_redis_connection") as mock_get_conn,
            patch("{{cookiecutter.module_name}}.storage.redis.app_settings") as mock_settings,
        ):
...
class TestRedisHandler:
    ...
    async def test_subscribe_new_channel(self):
        ...
        with (
            patch(
                "{{cookiecutter.module_name}}.storage.redis.get_redis_connection",
                new_callable=AsyncMock,
                return_value=mock_redis,
            ),
            patch(
                "{{cookiecutter.module_name}}.storage.redis.REventHandler.loop", new_callable=AsyncMock
            ),
        ):
...
class TestRRedis:
    ...
    def test_rredis_module_instance(self):
        ...
        from {{cookiecutter.module_name}}.storage.redis import r_redis

        # Import again to verify it's the same instance
        from {{cookiecutter.module_name}}.storage.redis import r_redis as r_redis2
