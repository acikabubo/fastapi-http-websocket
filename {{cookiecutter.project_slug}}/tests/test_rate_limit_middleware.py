...
class TestRateLimitMiddlewareDispatch:
    ...
    async def test_dispatch_when_disabled(
        ...
        """
        with patch("{{cookiecutter.module_name}}.middlewares.rate_limit.app_settings") as mock_settings:
    ...
    async def test_dispatch_rate_limit_allowed(
        ...
        with patch("{{cookiecutter.module_name}}.middlewares.rate_limit.app_settings") as mock_settings:
            ...
            with patch(
                "{{cookiecutter.module_name}}.middlewares.rate_limit.rate_limiter"
            ) as mock_limiter:
...
class TestGetRateLimitKey:
    ...
    def test_get_rate_limit_key_without_user(self, mock_app, mock_request):
        ...
        with patch("{{cookiecutter.module_name}}.middlewares.rate_limit.get_client_ip") as mock_get_ip:
