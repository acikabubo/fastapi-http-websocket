...
from {{cookiecutter.module_name}}.utils.profiling import (
    ProfilingManager,
...
class TestProfilingManager:
    ...
    def test_init_with_profiling_enabled_and_scalene_available(self):
        ...
        with (
            patch("{{cookiecutter.module_name}}.utils.profiling.app_settings") as mock_settings,
            patch("{{cookiecutter.module_name}}.utils.profiling.HAS_SCALENE", True),
            patch("{{cookiecutter.module_name}}.utils.profiling.Path.mkdir") as mock_mkdir,
        ):
    ...
    def test_init_with_scalene_not_installed(self):
        ...
        with (
            patch("{{cookiecutter.module_name}}.utils.profiling.app_settings") as mock_settings,
            patch("{{cookiecutter.module_name}}.utils.profiling.HAS_SCALENE", False),
            patch("{{cookiecutter.module_name}}.utils.profiling.logger") as mock_logger,
        ):
            mock_settings.PROFILING_ENABLED = True
            mock_settings.PROFILING_OUTPUT_DIR = "profiling_reports"
            mock_settings.PROFILING_INTERVAL_SECONDS = 30
    ...
    def test_get_report_path_default(self):
        ...
        with (
            patch("{{cookiecutter.module_name}}.utils.profiling.app_settings") as mock_settings,
            patch("{{cookiecutter.module_name}}.utils.profiling.HAS_SCALENE", True),
            patch("{{cookiecutter.module_name}}.utils.profiling.datetime") as mock_datetime,
        ):
            mock_settings.PROFILING_ENABLED = True
            mock_settings.PROFILING_OUTPUT_DIR = "profiling_reports"
            mock_settings.PROFILING_INTERVAL_SECONDS = 30
...
class TestProfileContext:
    ...
    async def test_profile_context_when_enabled(self):
        ...
        with (
            patch("{{cookiecutter.module_name}}.utils.profiling.profiling_manager") as mock_manager,
            patch("{{cookiecutter.module_name}}.utils.profiling.logger") as mock_logger,
        ):
...
class TestIsProfilingEnabled:
    ...
    def test_is_profiling_enabled_true(self):
        ...
        with patch("{{cookiecutter.module_name}}.utils.profiling.profiling_manager") as mock_manager:
            mock_manager.enabled = True
