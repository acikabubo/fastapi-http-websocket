...
from {{cookiecutter.module_name}}.logging import (
...
class TestGetCorrelationId:
    ...
    def test_get_correlation_id_returns_empty_string_on_runtime_error(
        ...
        with patch(
            "{{cookiecutter.module_name}}.middlewares.correlation_id.get_correlation_id",
...
class TestStructuredJSONFormatter:
    ...
    def test_format_includes_correlation_id_when_available(self):
        ...
        with patch(
            "{{cookiecutter.module_name}}.logging.get_correlation_id",
            return_value="correlation-123",
        ):
            formatted = formatter.format(record)
            log_data = json.loads(formatted)
...
class TestHumanReadableFormatter:
    ...
    def test_format_uses_dash_when_no_correlation_id(self):
        ...
        with patch("{{cookiecutter.module_name}}.logging.get_correlation_id", return_value=""):
            formatted = formatter.format(record)
...
class TestSetupLogging:
    ...
    def test_setup_logging_handles_file_handler_errors(self):
        ...
        with patch(
            "{{cookiecutter.module_name}}.logging.logging.FileHandler",
