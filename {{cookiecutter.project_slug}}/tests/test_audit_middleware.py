...
class TestAuditMiddleware:
    ...
    async def test_middleware_skips_excluded_paths(self, app):
        ...
        with patch(
            "{{cookiecutter.module_name}}.middlewares.audit_middleware.app_settings"
        ) as mock_settings:
    ...
    async def test_middleware_logs_successful_request(self, app, mock_user):
        ...
        with (
            ...
            patch(
                "{{cookiecutter.module_name}}.middlewares.audit_middleware.extract_ip_address"
            ) as mock_extract_ip,
            patch(
                "{{cookiecutter.module_name}}.middlewares.audit_middleware.log_user_action"
            ) as mock_log_action,
        ):
            mock_settings.EXCLUDED_PATHS.match.return_value = False
