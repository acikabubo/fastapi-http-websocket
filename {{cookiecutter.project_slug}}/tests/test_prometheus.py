...
class TestPrometheusMiddleware:
    ...
    async def test_middleware_tracks_successful_request(self, app):
        ...
        with (
            patch(
                "{{cookiecutter.module_name}}.middlewares.prometheus.http_requests_total"
            ) as mock_total,
            patch(
                "{{cookiecutter.module_name}}.middlewares.prometheus.http_request_duration_seconds"
            ) as mock_duration,
            patch(
                "{{cookiecutter.module_name}}.middlewares.prometheus.http_requests_in_progress"
            ) as mock_in_progress,
        ):
            # Setup label chain
