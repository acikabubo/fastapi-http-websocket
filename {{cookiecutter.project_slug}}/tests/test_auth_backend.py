...
class TestAuthBackendTokenCaching:
    ...
    async def test_cache_hit_skips_token_decode(self, mock_user_data):
        ...
        with patch("{{cookiecutter.module_name}}.auth.keycloak_manager", mock_kc_manager):
            # Mock cache hit - returns user data from cache
            with patch(
                "{{cookiecutter.module_name}}.utils.token_cache.get_cached_token_claims",
                new_callable=AsyncMock,
                return_value=mock_user_data,
            ):
                # Mock cache_token_claims to verify it's NOT called
                with patch(
                    "{{cookiecutter.module_name}}.utils.token_cache.cache_token_claims",
                    new_callable=AsyncMock,
                ) as mock_cache:
        ...
        with patch("{{cookiecutter.module_name}}.auth.keycloak_manager", mock_kc_manager):
            with patch(
                ...
                with patch(
                    ...
                    # Mock metrics
                    with patch(
                        "{{cookiecutter.module_name}}.utils.metrics.keycloak_token_validation_total"
                    ) as mock_validation:
                        with patch(
                            "{{cookiecutter.module_name}}.utils.metrics.auth_backend_requests_total"
                        ) as mock_backend:
                            with patch(
                                "{{cookiecutter.module_name}}.utils.metrics.keycloak_operation_duration_seconds"
                            ) as mock_duration:
