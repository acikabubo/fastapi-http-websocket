...
from {{cookiecutter.module_name}}.utils.audit_logger import (
    SENSITIVE_FIELDS,
    extract_ip_address,
    flush_audit_queue,
    get_audit_queue,
...
class TestExtractIpAddress:
    ...
    def test_extract_ip_from_client(self):
        ...
        with patch(
            "{{cookiecutter.module_name}}.utils.ip_utils.get_client_ip", return_value="192.168.1.100"
        ):
    ...
    def test_extract_ip_returns_none_when_unavailable(self):
        ...
        with patch("{{cookiecutter.module_name}}.utils.ip_utils.get_client_ip", return_value=None):
            ip = extract_ip_address(request)
...
class TestGetAuditQueue:
    ...
    def test_get_audit_queue_creates_queue(self):
        ...
        # Reset global queue
        import {{cookiecutter.module_name}}.utils.audit_logger

        {{cookiecutter.module_name}}.utils.audit_logger._audit_queue = None

        with patch("{{cookiecutter.module_name}}.utils.audit_logger.app_settings") as mock_settings:
    ...
    def test_get_audit_queue_returns_existing(self):
        ...
        {{cookiecutter.module_name}}.utils.audit_logger._audit_queue = existing_queue
...
class TestLogUserAction:
    ...
    async def test_log_user_action_queue_full(self):
        ...
        with (
            patch("{{cookiecutter.module_name}}.utils.audit_logger.app_settings") as mock_settings,
            patch("{{cookiecutter.module_name}}.utils.audit_logger.logger") as mock_logger,
        ):
    ...
    async def test_log_user_action_handles_exception(self):
        ...
        with (
            patch(
                "{{cookiecutter.module_name}}.utils.audit_logger.sanitize_data",
...
class TestFlushAuditQueue:
    ...
    async def test_flush_queue_handles_database_error(self):
        ...
        with (
            patch("{{cookiecutter.module_name}}.utils.audit_logger.app_settings") as mock_settings,
            patch("{{cookiecutter.module_name}}.utils.audit_logger.async_session") as mock_session_ctx,
            patch("{{cookiecutter.module_name}}.utils.audit_logger.logger") as mock_logger,
        ):
            mock_settings.AUDIT_QUEUE_MAX_SIZE = 1000
