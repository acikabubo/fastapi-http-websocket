...
protobuf-generate:
	@mkdir -p {{cookiecutter.module_name}}/schemas/proto
	@uv run python -m grpc_tools.protoc \
		-I=proto \
		--python_out={{cookiecutter.module_name}}/schemas/proto \
		--pyi_out={{cookiecutter.module_name}}/schemas/proto \
		proto/websocket.proto

protobuf-clean:
	@rm -rf {{cookiecutter.module_name}}/schemas/proto
```

#### 4. Create Converter Utilities
Create `{{cookiecutter.module_name}}/utils/protobuf_converter.py`:
```python
"""Utilities for converting between Pydantic and Protobuf messages."""
import json
from {{cookiecutter.module_name}}.schemas.proto import Request, Response
...
Modify `{{cookiecutter.module_name}}/api/ws/consumers/web.py`:
```python
...
Modify `{{cookiecutter.module_name}}/api/ws/websocket.py`:
```python
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("{{cookiecutter.module_name}}:application", host="0.0.0.0", port=8000)
```

#### 4. Add Settings (Optional)
```python
# {{cookiecutter.module_name}}/settings.py
...
scalene run --profile-only {{cookiecutter.module_name}}/api/ws/ run_server.py
