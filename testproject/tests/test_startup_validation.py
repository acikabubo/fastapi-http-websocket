...
class TestValidateDatabaseConnection:
    ...
    async def test_validate_database_connection_success(self):
        ...
        with patch("app.startup_validation.engine", mock_engine):
            with patch("app.startup_validation.app_settings") as settings:
...
class TestValidateRedisConnection:
    ...
    async def test_validate_redis_connection_success(self):
        ...
        with patch(
            "app.startup_validation.Redis", return_value=mock_redis
        ) as mock_redis_class:
            with patch("app.startup_validation.app_settings") as settings:
    ...
    async def test_validate_redis_connection_ping_fails(self):
        ...
        with patch("app.startup_validation.Redis", return_value=mock_redis):
            with pytest.raises(StartupValidationError) as exc_info:
                await validate_redis_connection()
...
class TestRunAllValidations:
    ...
    async def test_run_all_validations_success(self):
        ...
        with patch(
            "app.startup_validation.validate_settings"
        ) as mock_settings:
            with patch(
                "app.startup_validation.validate_database_connection"
            ) as mock_db:
                with patch(
                    "app.startup_validation.validate_redis_connection"
                ) as mock_redis:
