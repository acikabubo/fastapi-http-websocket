...
3. Request reaches endpoint handler in `app/api/http/`
...
- **PackageRouter** ([routing.py](app/routing.py)): Central routing system for WebSocket requests with validation and permission checking
- **AuthBackend** ([auth.py](app/auth.py)): Keycloak JWT authentication for HTTP and WebSocket
- **RBAC Manager** ([managers/rbac_manager.py](app/managers/rbac_manager.py)): Singleton managing role-based permissions from `actions.json`
- **Database** ([storage/db.py](app/storage/db.py)): PostgreSQL via SQLModel with pagination support
...
1. **Add PkgID** to [constants.py](app/api/ws/constants.py):
```python
class PkgID(IntEnum):
    MY_NEW_HANDLER = 10
```
...
Environment variables in [settings.py](app/settings.py):
...
- **[User Action Logging](docs/guides/USER_ACTION_LOGGING.md)** - Audit logging setup and usage
