...
# Redis Metrics
redis_operations_total = get_or_create_counter(
...
redis_operation_duration_seconds = get_or_create_histogram(
...
redis_pool_max_connections = get_or_create_gauge(
    "redis_pool_max_connections",
    "Maximum number of connections allowed in Redis pool",
    ["db"],
)

redis_pool_info = get_or_create_gauge(
    "redis_pool_info",
...
redis_pool_connections_created_total = get_or_create_gauge(
    "redis_pool_connections_created_total",
    "Total connections created in Redis pool",
    ["db"],
)

redis_pool_connections_in_use = get_or_create_gauge(
    "redis_pool_connections_in_use",
    "Current number of connections in use from Redis pool",
    ["db"],
)

redis_pool_connections_available = get_or_create_gauge(
...
rate_limit_hits_total = get_or_create_counter(
...
memory_cache_hits_total = get_or_create_counter(
    "memory_cache_hits_total",
    "Total memory cache hits (L1 cache)",
)

memory_cache_misses_total = get_or_create_counter(
    "memory_cache_misses_total",
    "Total memory cache misses (L1 cache)",
)

memory_cache_evictions_total = get_or_create_counter(
    "memory_cache_evictions_total",
    "Total memory cache evictions (LRU policy)",
)

memory_cache_size = get_or_create_gauge(
    "memory_cache_size",
