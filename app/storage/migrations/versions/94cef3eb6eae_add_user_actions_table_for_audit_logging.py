...
def upgrade() -> None:
    ...
    sa.Column('user_roles', postgresql.JSON(astext_type=sa.Text()), nullable=False),
    sa.Column('action_type', sqlmodel.sql.sqltypes.AutoString(length=100), nullable=False),
    sa.Column('resource', sqlmodel.sql.sqltypes.AutoString(length=500), nullable=False),
    ...
    op.create_index('idx_user_action', 'user_actions', ['user_id', 'action_type'], unique=False)
    op.create_index('idx_user_timestamp', 'user_actions', ['user_id', 'timestamp'], unique=False)
    op.create_index(op.f('ix_user_actions_action_type'), 'user_actions', ['action_type'], unique=False)
    op.create_index(op.f('ix_user_actions_outcome'), 'user_actions', ['outcome'], unique=False)
    op.create_index(op.f('ix_user_actions_request_id'), 'user_actions', ['request_id'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_user_actions_request_id'), table_name='user_actions')
